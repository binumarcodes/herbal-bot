  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100vh", backgroundColor: "#f0fdf4" }}>
      ...
      <header style={{
        ...
        <span style={{ fontWeight: "bold", fontSize: "1.2rem" }}>🌿 Herbal Chatbot</span>
        <button
          onClick={logout}
          style={{
            background: "#f97316",
      ...
      {/* Chat Messages */}
      <main style={{ flex: 1, padding: "1rem 2rem", overflowY: "auto", display: "flex", flexDirection: "column", gap: "0.75rem" }}>
        {chat.length === 0 && !typing && (
          <p style={{ color: "#16a34a" }}>👋 Hi {user}! You can say hello or type a symptom: fever, headache, cough, stomach pain, body ache.</p>
        )}
        {chat.map((msg, idx) => (
          <div
            key={idx}
      ...
      {/* Input Form */}
      <form onSubmit={handleSend} style={{ display: "flex", padding: "1rem 2rem", gap: "0.5rem", borderTop: "1px solid #e5e7eb", backgroundColor: "#ffffff" }}>
        <input
          value={message}
